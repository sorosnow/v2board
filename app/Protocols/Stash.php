...
class Stash
{
    ...
    public function handle()
    {
        ...
        foreach ($servers as $item) {
            ...
            if ($item['type'] === 'tuic') {
                array_push($proxy, self::buildTuic($uuid, $item));
                array_push($proxies, $item['name']);
            }
            if ($item['type'] === 'hysteria') {
                array_push($proxy, self::buildHysteria($uuid, $item));
                array_push($proxies, $item['name']);
            }
            if ($item['type'] === 'hysteria2') {
                array_push($proxy, self::buildHysteria2($uuid, $item));
                array_push($proxies, $item['name']);
            }
            if ($item['type'] === 'anytls') {
                array_push($proxy, self::buildAnyTLS($uuid, $item));
                array_push($proxies, $item['name']);
            }
        }
